...
const target = `{messages.map(msg => {
                  ...
                  <div key={msg.id} className={cn(
                    ...
                    <div className={cn(
                      "max-w-[85%] px-4 py-2.5 text-[15px] relative shadow-md transition-all whitespace-pre-wrap leading-relaxed inline-block",
                      isAI 
...
const replacement = `{messages.map(msg => {
                  ...
                  return (
                  <div key={msg.id} className={cn(
                    "flex group relative w-full mb-5",
                    isAI ? "justify-center" : (isMe ? "justify-start" : "justify-end")
                  )}>
                    <div className={cn(
                      ...
                      isAI 
                        ...
                        : (isMe
                          ? "bg-brand-green text-white rounded-[18px] rounded-tr-[4px] px-4 py-2 text-[15px]" 
                          : "bg-[#27272a] text-gray-100 rounded-[18px] rounded-tl-[4px] border border-white/5 px-4 py-2 text-[15px]")
                    )}>
                      {isAI && (
                        <div className="flex items-center gap-2 mb-3 text-indigo-400 font-bold border-b border-indigo-500/20 pb-2">
                          <Bot size={16} />
...
if (content.includes(target)) {
  ...
  const targetFallback = target.replace(' inline-block', '');
  if (content.includes(targetFallback)) {
    content = content.replace(targetFallback, replacement);
    fs.writeFileSync('src/pages/Messages.tsx', content);
    console.log("REPLACED SUCCESSFULLY WITH FALLBACK");
  } else {
